...
function Grass() {
    ...
    const middle = createSection(0xff8c00);
    middle.receiveShadow = true;
    grass.add(middle);

    const left = createSection(0xd2691e);
    left.position.x = -boardWidth * zoom;
    grass.add(left);

    const right = createSection(0xd2691e);
    right.position.x = boardWidth * zoom;
    grass.add(right);
...
function Lane(index) {
    this.index = index;
    this.type =
        index <= 0 ? "field" : laneTypes[Math.floor(Math.random() * laneTypes.length)];
